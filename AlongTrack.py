...
class AlongTrackDatabase:
	...
	def create_along_track_metadata_table(self):
		...
		with pg.connect(self.connect_string()) as conn:
			with conn.cursor() as cur:
				...
				try: # Postgres does not have a IF DOES NOT EXIST for constraints. We'll catch the error and move on if it already exists
					cur.execute(add_primary_key_query)
				except:
					print('cop_meta_pkey not created because it already exists.')
				conn.commit()
		print(f"Table 'cop_meta' added to database (if it did not previously exist) '{self.dbName}'.")
	...
	def import_metadata_to_psql(self, ds, fname):
		...
		with pg.connect(self.connect_string()) as conn:
			with conn.cursor() as cur:
				data = (fname, ds.Conventions, ds.Metadata_Conventions, ds.cdm_data_type, ds.comment, ds.contact, ds.creator_email,
		ds.creator_name, ds.creator_url, ds.date_created, ds.date_issued, ds.date_modified, ds.history, ds.institution,
		ds.keywords, ds.license, ds.platform, ds.processing_level, ds.product_version, ds.project, ds.references,
	...
	def extract_data_from_netcdf(self, file_path, fname):
		...
		try:
			...
			print("File '{}' not found".format(directory))

	# Define the function to import data into PostgreSQL using copy_from in binary format
	def import_data_to_postgresql(self, fname, time_data, lat_data, lon_data, cycle_data, track_data, sla_un_data, sla_f_data,
		...
		with pg.connect(self.connect_string()) as conn:
			with conn.cursor() as cur:
				...
				copy_query = sql.SQL("COPY {along_tbl_nme} ( nme, track, cycle, lat, lon, sla_unfiltered, sla_filtered, date_time, dac, ocean_tide, internal_tide, lwe, mdt, tpa_correction) FROM STDIN WITH (FORMAT binary);").format(along_tbl_nme=sql.Identifier(self.alongTrackTableName))
		...
		with pg.connect(self.connect_string()) as conn:
			with conn.cursor() as cur:
				for date in dates:
					...
					query = sql.SQL("CREATE TABLE IF NOT EXISTS {partition_nm} PARTITION OF {table_name} FOR VALUES FROM ('{min_partition_date}') TO ('{to_partition}');").format(
						table_name=sql.Identifier(self.alongTrackTableName),
	...
	def insert_along_track_data_from_netcdf(self, directory):
		...
		for file_path in glob.glob(directory + '/*.nc'):
			...
			time_data, lat_data, lon_data, cycle_data, track_data, sla_un_data, sla_f_data, dac_data, o_tide_data, i_tide_data, lwe_data, mdt_data, tpa_corr_data = self.extract_data_from_netcdf(file_path, fname)
			import_start = time.time()
			self.import_data_to_postgresql(fname, time_data, lat_data, lon_data, cycle_data, track_data, sla_un_data,
									  sla_f_data, dac_data, o_tide_data, i_tide_data, lwe_data, mdt_data, tpa_corr_data)
