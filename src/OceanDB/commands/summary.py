...
from OceanDB.commands.output import format_status_line, render_table, style_value
...
@summary_group.command("along-track")
def summarize_along_track_command():
    """Summarize ingested along-track missions and their date coverage."""
    _render_along_track_summary()


@summary_group.command("alongtrack", hidden=True)
def summarize_alongtrack_hidden_alias():
    """Backward-compatible alias for along-track summary."""
    _render_along_track_summary()


@click.group("along_track", hidden=True)
def along_track_group():
    """Backward-compatible along-track command group."""


@along_track_group.command("summary", hidden=True)
def summarize_along_track_hidden_group_alias():
    """Backward-compatible alias for along-track summary."""
    _render_along_track_summary()
