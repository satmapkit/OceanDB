...
@click.option("--show-definition", is_flag=True, help="Include each index definition from pg_indexes.")
@click.option(
    "--all",
    "include_all",
    is_flag=True,
...
@click.option("--all", "drop_all", is_flag=True, help="Drop all OceanDB-managed indices.")
...
def drop_index_command(drop_all: bool, yes: bool):
    ...
    if not yes:
        proceed = click.confirm("Drop all OceanDB-managed indices from the configured database?")
        if not proceed:
